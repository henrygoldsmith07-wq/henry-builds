 ...
 * too narrowly, the import keeps the committed data and source verification
 * can degrade. This check runs immediately after the import so degradation
 * fails the sync job instead.
 *
...
function fail(problems) {
  ...
  console.error(
    "\nFix: point the REGISTRY_TOKEN secret at a PAT that can read the " +
      "sibling repos, then re-run this workflow. Until then the site may be " +
