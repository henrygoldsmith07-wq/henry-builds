...
export default function Landing() {
  ...
  const catalogFilters = useMemo(() => ["All", ...Array.from(new Set(projects.map((project) => project.category.split(" /")[0])))], []);
  const catalogProjects = useMemo(() => {
    const normalizedQuery = catalogQuery.trim().toLowerCase();
    return projects.filter((project) => {
      const matchesFilter = catalogFilter === "All" || project.category.startsWith(catalogFilter);
      const searchableText = [project.name, project.description, project.category, project.status, ...project.tags].join(" ").toLowerCase();
    ...
    <div className="portfolio-shell min-h-screen overflow-x-hidden bg-background text-foreground">
      <header className={`site-nav ${scrolled ? "site-nav-scrolled" : ""}`}>
        <div className="mx-auto flex max-w-[1380px] items-center justify-between px-5 py-4 sm:px-8 lg:px-12">
          <a href="#top" className="group flex items-center gap-3" aria-label="Portfolio website home"><span className="monogram">HG</span><span className="hidden text-sm font-semibold tracking-tight sm:block">Portfolio website</span></a>
